"""
Standalone Streamlit dashboard for the soft-sensor demo.

Self-contained: all imports are local to this directory. Designed to run
as the root of a Hugging Face Spaces deployment.
...
# Local imports — inference layer and simulator are siblings of this file
...
from inference import (
    ...
    DEBUTANIZER_HISTORY_LENGTH,
    FERMENTATION_HISTORY_LENGTH,
)
...
def _initialize_models():
    """Load models once at first call, then cache forever."""
    return load_all_models()


@st.cache_data
def load_debutanizer_data():
    """Load the bundled Debutanizer fixture."""
    return pd.read_csv(HERE / "example_data" / "Debutanizer_Data.txt", sep=r"\s+")


@st.cache_data
def load_fermentation_data():
    """Generate the simulated fermentation campaign (40 batches, seed=42).
    Cached after first call so we only pay this cost on cold start.
    """
    from simulator.fermentation import simulate_campaign

    return simulate_campaign(n_batches=40, duration_h=200.0, dt=0.5, seed=42)


# --------------------------------------------------------------------------
# Initialize models
# --------------------------------------------------------------------------
_load_results = _initialize_models()
...
with st.sidebar:
    ...
    if all(loaded.values()):
        ...
        bad = [k for k, v in loaded.items() if not v]
        st.error(f"Models failed to load: {bad}")

    st.divider()
    st.caption(
        "[GitHub](https://github.com/beebzy-droid/soft-sensors-continuous-processes) · "
        "MIT License"
    )
...
with tab_debutanizer:
    ...
    with col_slider:
        ...
        sample_idx = st.slider(
            "Sample index ('now')",
            min_value=DEBUTANIZER_HISTORY_LENGTH,
            max_value=max_idx,
            value=2200,
            step=10,
        )
    ...
    history_window = df_debut.iloc[
        sample_idx - DEBUTANIZER_HISTORY_LENGTH + 1 : sample_idx + 1
    ]
    ...
    try:
        result = predict_debutanizer(history_dict)
...
with tab_fermentation:
    ...
    with st.spinner("Generating fermentation campaign (first load only)..."):
    ...
    min_time = batch_df["time_h"].iloc[FERMENTATION_HISTORY_LENGTH]
    ...
    history_window = batch_df.iloc[
        now_idx - FERMENTATION_HISTORY_LENGTH + 1 : now_idx + 1
    ]
    ...
    try:
        result = predict_fermentation(actual_time, history_dict)
        prediction = result["prediction"]
        lower = result["lower_95"]
        upper = result["upper_95"]
        in_interval = lower <= actual_y <= upper
